...
class RBPNDataset():
    ...
    def __getitem__(self, index):
        ...
        if self.patch_size != 0:
            x, target, neighbor, _ = get_patch(x, target, neighbor, self.patch_size, self.upscale_factor,
                                               self.nFrames)

        if self.data_augmentation:
            x, target, neighbor, _ = augment(x, target, neighbor)
        ...
        return target, x, neighbor_all, flow
...
def create_train_dataset(dataset, args):
    ...
    if parallel_mode in [ParallelMode.DATA_PARALLEL, ParallelMode.HYBRID_PARALLEL]:
        train_ds = de.GeneratorDataset(dataset, num_parallel_workers=args.threads,
                                       column_names=['target_image', 'input_image', 'neighbor_image',
                                                     'flow_image'],
